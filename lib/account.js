...
function Account(options) {
  ...
  try {
    provider = require('./providers/' + this.provider);
  } catch (err) {
    options = {};
    this.provider = process.env.PROVIDER || 'github';
    provider = require('./providers/' + this.provider);
  }
  this.repo = options.repo || process.env.REPO;
  this.secret = options.secret || process.env.SECRET || process.env.PERSONAL_ACCESS_TOKEN;
  this.label = options.label || process.env.LABEL || process.env.ISSUE_LABEL || 'bug';
