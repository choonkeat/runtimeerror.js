  ...
  handle: function(req, res, callback) {
    ...
    inflate.on('end', function() {
      try {
        ...
        // remove known secrets
        if (payload.application_environment && payload.application_environment.env) delete payload.application_environment.env.RUNTIMEERROR_EMAIL;
        runtimeerror.handle(account, payload.exception.exception_class + ': ' + payload.exception.message, runtimeerror.json2htmltables(payload), function(err, attrs) {
