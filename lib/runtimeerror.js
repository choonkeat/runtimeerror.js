...
module.exports = {
  ...
  json2markdown: function(json) {
    ...
    for (var k in json) {
      ...
      if ((typeof v) == 'object') {
        chunks.push('</table>');
        chunks.push('<h4>' + htmlescape(k) + '</h4>');
        if (v.length == undefined) {
          chunks.push(runtimeerror.json2markdown(v));
        } else {
          chunks.push('<pre>' + htmlescape(v.join("\n")) + '</pre>\n');
        }
        chunks.push('<table>');
      } else {
        chunks.push('<tr><th align="left">' + htmlescape(k) + '</th><td align="left">' + htmlescape(v) + '</td></tr>');
      }
    }
    chunks.push('</table>');
    return chunks.join('');
