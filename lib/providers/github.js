...
Provider.prototype.mail_options = function(email, attrs) {
  var options = {
    ...
    messageId:  this.repo.name + "/issues/" + attrs.number + "@github." + this.secret + "." + (new Date()).getTime(),
...
Provider.prototype.create_issue = function(attrs, callback) {
  this.repo.create_issue({ title: attrs.title, body: attrs.body, labels: this.label.split(/,\s*/) }, callback);
};

Provider.prototype.update_issue = function(uid, attrs, callback) {
  this.repo.edit_issue(uid, { title: attrs.title, body: attrs.body, labels: attrs.labels }, callback);
};

Provider.prototype.reopen_issue = function(uid, attrs, callback) {
  var that = this;
  that.repo.edit_issue(uid, { state: 'open' }, function() {
    that.comment_issue(uid, attrs, callback);
  });
};

Provider.prototype.comment_issue = function(uid, attrs, callback) {
  var that = this;
